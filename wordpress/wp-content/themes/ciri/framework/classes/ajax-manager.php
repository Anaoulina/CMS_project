...
if (!class_exists('Ciri_Ajax_Manager')) {
    ...
    class Ciri_Ajax_Manager
    {
	    ...
	    public function handle_ajax_request() {
		    if ( ! $this->verify_request_nonce() ) {
			    ...
			    $this->add_response_data( false, esc_html__( 'Token Expired.', 'ciri' ) )
			         ->send_error( 401 );
		    }

		    if(empty($_REQUEST['actions'])){
			    $this->add_response_data( false, esc_html__( 'Action not found.', 'ciri' ) )
			         ->send_error( 401 );
		    }
		    ...
		    foreach ( $this->requests as $id => $action_data ) {
			    ...
			    if ( ! isset( $this->ajax_actions[ $action_data['action'] ] ) ) {
				    $this->add_response_data( false, esc_html__( 'Action not found.', 'ciri' ), 400 );
	    ...
	    private function send_success() {
		    ...
		    if ( function_exists( 'gzencode' ) ) {
			    ...
			    echo ciri_render_variable($response); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		    } else {
			    echo ciri_render_variable($json); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		    }
	     ...
	     *
	     * @return Ciri_Ajax_Manager An instance of ajax manager.
	     */
